...
class nts_RNN(nn.Module):
    def __init__(self, hidden_size, fut_len):
        super().__init__()
        ...
        self.linear = nn.Linear(in_features=self.hidden_size[num_hidden_layer-1], out_features=fut_len)
...
class nts_MLP(nn.Module):
    def __init__(self, hidden_size, input_size, fut_len):
        super().__init__()
        # hidden_size should be a list with 1-3 elements
        self.hidden_size = hidden_size
        num_hidden_layer = len(hidden_size)
